<div>
    @forelse($mostReadArticles as $index => $article)
        <div class="flex items-start gap-4 pb-4 {{ !$loop->last ? 'border-b border-slate-200' : '' }} mb-4 group">
            <div class="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-orange-500 to-red-600 text-white rounded-full flex items-center justify-center text-sm font-bold shadow-lg">
                {{ $index + 1 }}
            </div>
            
            <div class="flex-1 min-w-0">
                <h4 class="font-semibold text-slate-800 text-sm leading-tight mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors">
    ...
    @empty
        <div class="text-center py-8">
            ...
            <p class="text-slate-500 text-sm">سيتم عرض المقالات الأكثر قراءة هنا</p>
        </div>
    @endforelse
</div>